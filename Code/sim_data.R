rm(list = ls())
set.seed(010101)
source('After/ABC4SFAv2/Code/Fixed/requirements.R')
source('After/ABC4SFAv2/Code/Fixed/functions.R')
...
sigmas <- readxl::read_excel('After/ABC4SFAv2/Data/Inputs/Badunenko&Kumbhakar.xlsx')
...
for (j in 1:nrow(sigmas)){
  ...
  for (i in 1:theta$nsim){
    sfm <- SFM_all(X, theta, dist = theta$model)
...
saveRDS(ABC_data, sprintf('After/ABC4SFAv2/Data/Inputs/%s_simData.RData', filename))
saveRDS(ABC_Ystats, sprintf('After/ABC4SFAv2/Data/Inputs/%s_Ystats.RData', filename))
